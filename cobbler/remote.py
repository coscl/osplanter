...
class CobblerXMLRPCInterface:
    ...
    def background_reboot(self, options, token):
        def runner(self):
            ssh = paramiko.SSHClient()
            ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
            ssh.connect(self.options.get("ip", None), 22, "root" ,self.options.get("passwd", None))
            stdin, stdout, stderr = ssh.exec_command("reboot")
            ssh.close()
            return True
        return self.__start_task(runner, token, "reboot", "Host reboot", options)
